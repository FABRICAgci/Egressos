    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body mt-2">
          <form class="dt_adv_search" action="{{ route('index-administrador') }}" method="POST">
            @csrf
            <div class="row g-1 mb-md-1">
              <div class="col-md-4">
                  ...
                  @if (!empty($filtros['ativo']) && !empty($filtros['name']))
                    value="{{ $filtros['name'] }}"
                  @else
                    value=""
                  @endif
          ...
          @if ($administradores->isNotEmpty())
              @foreach($administradores as $a)
                <p class="card-text">
                  ...
                  <a href="{{ route('edit-administrador', $a->id) }}">
                  ...
                  <a href="{{ route('destroy-administrador', $a->id) }}" onclick='return confirm("Deseja mesmo excluir {{$a->name}}?");'>
                ...
                <p>
                  <div class=@if($a->ativo == 1) "card-text fst-italic text-info" @else "card-text fst-italic text-secondary" @endif>
                    <i data-feather="chevrons-right"></i>
                    {{ $a->name }}
                  </div>
                </p>

                <div class=@if($a->ativo == 1) "divider divider-info" @else "divider divider-secondary" @endif>
...
<div>
  @if (!empty($filtros)) {{$administradores->appends($filtros)->links()}} @else {{$administradores->links()}} @endif
</div>
